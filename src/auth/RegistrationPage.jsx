...
function RegistrationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await UserService.AdminRegistration(formData, token);
  ...
  return (
    <motion.div className="flex  items-center justify-center min-h-screen" initial="hidden" animate="visible" variants={containerVariants}>
      <motion.div className="bg-green-400 p-8  bg-opacity-30 backdrop-filter backdrop-blur-lg  rounded shadow-md w-full max-w-md" initial="hidden" animate="visible" variants={containerVariants}>
        <motion.h2 className="text-2xl font-bold mb-6 text-center" variants={itemVariants}>ثبت‌نام</motion.h2>
        <motion.form onSubmit={handleSubmit} variants={itemVariants}>
          ...
          <motion.div className="mb-4" variants={itemVariants}>
            ...
            <input type="password" name="password" value={formData.password} onChange={handleInputChange} required className="w-full px-3 py-2 border rounded" />
          </motion.div>
          <motion.div className="mb-4" variants={itemVariants}>
          ...
          <motion.button type="submit" className="w-full bg-blue-500 text-white py-2 rounded hover:bg-blue-600" variants={itemVariants}>ثبت‌نام</motion.button>
        </motion.form>
      </motion.div>
    </motion.div>
  );
